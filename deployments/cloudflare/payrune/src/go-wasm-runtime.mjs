...
async function initializeRuntime() {
  ...
  const { default: wasmModule } = await import('./generated/payrune_worker.wasm');
  ...
  for (let attempts = 0; attempts < 50; attempts += 1) {
    if (typeof globalThis.payruneWorkerHandle === 'function') {
      return {
        async handle(operation, encodedRequest) {
          return globalThis.payruneWorkerHandle(operation, encodedRequest);
        },
      };
    }
